/// <mls fileReference="_102021_/l2/agentEndpointLayer2Controller.ts" enhancement="_blank" />
...
const agentName = "agentEndpointLayer2Controller";
...
export function createAgent(): IAgent {
    return {
        ...
        agentDescription: "Agent agentEndpointLayer2Controller, for create controller",
        visibility: "private",
...
const _beforePrompt = async (context: mls.msg.ExecutionContext): Promise<void> => {
    ...
    if (!context.task) {
        let prompt = context.message.content.replace('@@agentEndpointLayer2Controller', '').trim();
...
async function nextStep(context: mls.msg.ExecutionContext) {
    ...
    const newStep: mls.msg.AIPayload = {
        agentName: 'agentEndpointLayer2Routes',
        prompt: 'ok',
        status: 'pending',
...
async function getPrompts(userPrompt: string): Promise<mls.msg.IAMessageInputType[]> {
    ...
    const dataForReplace = {
        userPrompt,
        source: await getEntity(info.endpoint),
    }
...
async function getEntity(shortName: string) {
    ...
    const key = mls.stor.getKeyToFiles(mls.actualProject || 0, 1, lowercaseFirstLetter(shortName), 'layer_2_controllers', '.ts');
