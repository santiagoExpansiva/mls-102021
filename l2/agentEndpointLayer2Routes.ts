/// <mls fileReference="_102021_/l2/agentEndpointLayer2Routes.ts" enhancement="_blank" />
...
const agentName = "agentEndpointLayer2Routes";
...
export function createAgent(): IAgent {
    return {
        ...
        agentDescription: "Agent agentEndpointLayer2Routes, for create tipe context",
        visibility: "private",
...
const _beforePrompt = async (context: mls.msg.ExecutionContext): Promise<void> => {
    ...
    if (!context.task) {
        let prompt = context.message.content.replace('@@agentEndpointLayer2Routes', '').trim();
...
async function nextStep(context: mls.msg.ExecutionContext) {
    ...
    const newStep: mls.msg.AIPayload = {
        agentName: 'agentEndpointLayer1localDB',
        prompt: 'ok',
        status: 'pending',
...
async function getRoutes() {
    ...
    const key = mls.stor.getKeyToFiles(mls.actualProject || 0, 1, 'routes', 'layer_2_controllers', '.ts');
