/// <mls fileReference="_102021_/l2/agentEndpointLayer3UseCase.ts" enhancement="_blank" />
...
const agentName = "agentEndpointLayer3UseCase";
...
export function createAgent(): IAgent {
    return {
        ...
        agentDescription: "Agent agentEndpointLayer3UseCase, for create use case",
        visibility: "private",
...
const _beforePrompt = async (context: mls.msg.ExecutionContext): Promise<void> => {
    ...
    if (!context.task) {
        let prompt = context.message.content.replace('@@agentEndpointLayer3UseCase', '').trim();
...
async function nextStep(context: mls.msg.ExecutionContext) {
    ...
    const newStep: mls.msg.AIPayload = {
        agentName: 'agentEndpointLayer2Controller',
        prompt: 'ok',
        status: 'pending',
...
async function getEntity(shortName: string) {
    ...
    const key = mls.stor.getKeyToFiles(mls.actualProject || 0, 1, lowercaseFirstLetter(shortName), 'layer_3_use_cases', '.ts');
